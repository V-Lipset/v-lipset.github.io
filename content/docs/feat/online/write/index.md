...
* 注意：此处所列各项都是用来描述**术语表**的基本信息的。例如：版本号指的是术语表的版本号（一般从 `1.0.0` 或 `0.1.0` 开始），而不是 AO3 Translator 的版本号。
...
* **注意**：
  * 正则表达式功能非常强大，但也极具风险。一个编写不当的表达式可能会严重影响浏览器性能，甚至导致页面卡死。请仅在您完全理解正则表达式并经过充分测试后才使用此功能。
  * 如果编写好的正则表达式不起作用，请在插件的 `更多功能` - `调试模式与日志` 选项中 `导出运行日志` ，查看日志文件以分析出现问题的具体原因。
